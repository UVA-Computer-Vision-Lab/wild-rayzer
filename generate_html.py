...
def generate_html(result_folder, output_file="result_visualization.html"):
    ...
    # Get all sample directories (they can be named with numbers or scene names)
    sample_dirs = [
        d
        for d in os.listdir(result_folder)
        if os.path.isdir(os.path.join(result_folder, d)) and not d.startswith(".")
    ]
    # Sort: numeric dirs first (numerically), then scene names (alphabetically)
    sample_dirs.sort(key=lambda x: (not x.isdigit(), int(x) if x.isdigit() else x))
    ...
    def get_nav_html(current_page):
        ...
        if num_pages == 1:
            ...
            next_link = (
                f"{base_name}_page_{current_page+1}{ext}" if current_page < num_pages else ""
            )
        ...
        for sample_dir in page_samples:
            ...
            if os.path.exists(metrics_path):
                try:
                    ...
                    metrics_html = (
                        f"<div class='error-metrics'>Error reading metrics: {str(e)}</div>"
                    )
