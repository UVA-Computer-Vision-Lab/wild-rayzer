...
class WildRayZerDemo:
    ...
    def create_video(self, frames, fps=30):
        """Write frames to a browser-playable H.264 MP4.

        `frames` is [V, 3, H, W] float in [0, 1].
        Uses torchvision.io (ffmpeg, H.264 / yuv420p) so the result plays in
        any modern browser — cv2's default mp4v codec produces MPEG-4 Part 2
        which Chromium/Firefox refuse to play inline.
        """
        import torchvision.io as tvio

        frames_u8 = (frames.clamp(0, 1) * 255).to(torch.uint8)  # [V, 3, H, W]
        frames_u8 = frames_u8.permute(0, 2, 3, 1).contiguous()  # [V, H, W, 3]
        ...
        try:
            tvio.write_video(
                ...
                fps=fps,
                video_codec="h264",
                options={"crf": "20", "pix_fmt": "yuv420p"},
            )
            return video_path
        except Exception as e:
            print(f"[create_video] torchvision write_video failed: {e}")
            # Fallback to cv2 mp4v (may not play in browser).
            frames_np = frames_u8.numpy()
