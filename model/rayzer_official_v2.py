...
class MotionMaskPredictor(nn.Module):
    ...
    def __init__(self, config, dinov3_backbone, dinov3_processor):
        ...
        self.head = nn.Sequential(
            nn.Conv2d(d_fused // 32, d_fused // 32, kernel_size=3, padding=1),
            nn.GELU(),
            nn.Conv2d(d_fused // 32, 1, kernel_size=1, bias=False),
        )
...
class DinoV3UncertaintyPseudoLabelMaker(nn.Module):
    ...
    def _extract_patch_features(self, img_tensor):
        ...
        assert H_patches * W_patches == N, f"Cannot reshape {N} patches into square grid"
    ...
    def generate_binary_coseg_masks(
        self, gt_img, pred_img, save_dir=None, frame_indices=None
    ) -> torch.Tensor:
        """
    ...
    @torch.no_grad()
    def forward(self, gt_img, pred_img, debug_save_path=None):
        """
...
class Images2Latent4D(nn.Module):
    ...
    def forward(self, data, create_visual=False, render_video=False, iter=0):
        ...
        plucker_emb_input = self.target_pose_tokenizer(plucker_input)  # [b * v_input, n, d]
        plucker_emb_target = (
        ...
        # Predict motion masks from three modalities (DINO + Tokens + Pose)
        predicted_masks_target = None
        dinov3_masks = None
        if self.use_motion_mask and self.motion_mask_predictor is not None and v_target > 0:
            ...
            # Predict motion masks
            predicted_masks_target = self.motion_mask_predictor(
                target_images_for_motion, target_image_tokens, plucker_emb_target
        ...
        if plucker_emb_target is not None and v_target > 0:
            ...
            # No target views to render - create dummy loss for compatibility
        ...
        if create_visual and v_target > 0:
            with torch.no_grad():
